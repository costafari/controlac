...
public class ProveedoresResource {
    ...
    public ResponseEntity<Proveedores> partialUpdateProveedores(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, proveedores.getId().toString())
        );
    }
    ...
    public List<Proveedores> getAllProveedores(@RequestParam(required = false) String filter) {
        if ("lotes-is-null".equals(filter)) {
            log.debug("REST request to get all Proveedoress where lotes is null");
            return StreamSupport
                .stream(proveedoresRepository.findAll().spliterator(), false)
