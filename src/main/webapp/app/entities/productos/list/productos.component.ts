...
export class ProductosComponent implements OnInit {
  ...
  protected refineData(data: IProductos[]): IProductos[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IProductos[] | null): IProductos[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.productosService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
